...
export async function setImageAcrossMonitors(
    ...
    await runExternalScripts(imageFilePath);
...
async function runExternalScripts(imageFilePath: string) {
    for (let idx = 0; idx < configuration.scripts.length; idx++) {
        const script = configuration.scripts[idx];
        try {
            await execPomisified(`"${script}" "${imageFilePath}"`);
        } catch (error) {
            logger.error(error);
            notify(
